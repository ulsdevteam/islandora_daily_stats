...
# The models and names for them are stored in a mysql table
...
foreach ($cmodels as $model_id=>$cm) {
        ...
        $query_pitt    = "select?q=RELS_EXT_hasModel_uri_s:*$cm*%20AND%20PID:" . ISLANDORA_PREFIX . "\:*&fl=PID&rows=0";
        $query_nonpitt = "select?q=RELS_EXT_hasModel_uri_s:*$cm*%20AND%20!PID:" . ISLANDORA_PREFIX . "\:*&fl=PID&rows=0";
        ...
        $label_nonpitt = $cmodel_desc[$model_id]." (non " . ISLANDORA_PREFIX . ":)";
